...
grep -q '"arbitrary_urls": false' "$ROOT/config/plugin-sources.json" || fail "plugin source URL policy changed"
grep -q '"arbitrary_shell": false' "$ROOT/config/plugin-sources.json" || fail "plugin source shell policy changed"
grep -q '"installation_mode": "receiver_source"' "$ROOT/config/plugin-sources.json" || fail "plugin source installation mode changed"
