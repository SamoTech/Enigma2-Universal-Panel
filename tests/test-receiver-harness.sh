...
printf 'Mock receiver harness completed. No real receiver was contacted.\n'
 "$ROOT/install.sh" || fail "installer one-line command missing"
grep -q 'installer v1.6.0' "$ROOT/install.sh" || fail "installer version marker missing"
grep -q 'mktemp -d /tmp/e2panel-install' "$ROOT/install.sh" || fail "installer staging directory missing"
grep -q -- '--tries=' "$ROOT/install.sh" || fail "wget retry policy missing"
grep -q -- '--retry ' "$ROOT/install.sh" || fail "curl retry policy missing"
grep -q 'Downloaded file is empty' "$ROOT/install.sh" || fail "installer empty-download guard missing"
grep -q 'Installation files deployed' "$ROOT/install.sh" || fail "installer completion marker missing"
pass "installer command, staging, retry and integrity guards"
